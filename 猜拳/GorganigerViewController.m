...
@interface GorganigerViewController ()

@end

@implementation GorganigerViewController
...
-(void)MyButon:(UIButton *)but
{
    ...
    switch (but.tag) {
        ...
        {
            NSLog(@"胜者");
            [defaults setObject:@"1" forKey:@"guessflag"];
            [defaults synchronize];//用synchronize方法把数据持久化到standardUse

            //_Return.userInteractionEnabled = NO;
            //RQIdentifierForStartfingerguess
            ident = RQIdentifierForGetguessflag;
            upLoadRequest = [[ASIFormDataRequest alloc] initWithURL:[NSURL URLWithString:GetguessflagURL]];
            ...
            [upLoadRequest setPostValue:@"1" forKey:@"guessflag"];
        ...
        {
            NSLog(@"败者");
            [defaults setObject:@"0" forKey:@"guessflag"];
            [defaults synchronize];//用synchronize方法把数据持久化到standardUse
            ...
            //[upLoadRequest setPostValue:@"1" forKey:@"isgameover"];
            //[upLoadRequest setPostValue:[defaults objectForKey:@"userID"] forKey:@"gameuserid"];
            [upLoadRequest setPostValue:@"0" forKey:@"guessflag"];
...
- (void)requestFinished:(ASIHTTPRequest *)request
{
    ...
    switch (ident) {
        case RQIdentifierForGetguessflag:
        {//开始
            ...
            if([[dictionary objectForKey:@"createflag"] integerValue] == 1)
            {
                [self openMybutton];
        ...
        case RQIdentifierForContinueguess:
        {
            ...
            if([dictionary objectForKey:@"sum"] != nil)
            {
                [defaults setObject:[dictionary objectForKey:@"sum"] forKey:@"sum"];
                [self openMybutton];
            }
...
-(void)Continuetime
{
    ...
    NSLog(@"加载结果list  guessflag = %@",[defaults objectForKey:@"guessflag"]);
